		...
		if(!toplevel) {
			...
			if(bcar instanceof Cons) {
				...
				SyntaxUtils.compileList(
						bcdr, env, comp, nbuild, b2, true,
						mesg, new ArrayList<Cons>());
				//symlist.remove(0);
				nbuild.addReturnOp();

				// create macro object and define it
				Macro cl = new Macro(
						b2.getCdr(), nbuild.getCodeRef(), env);

				if(!(b2.getCar() instanceof Symbol)) {
					throw mesg.getError("err.symbol", b2.getCar());
				}
				env.bindDatum(b2.getCar(), cl);
				build.addPush(Undef.UNDEF);
			} else if(bcar instanceof Symbol ||
					bcar instanceof SymbolScope) {
				Symbol sym;

				if(bcar instanceof Symbol) {
					sym = (Symbol)bcar;
				} else {
					sym = ((SymbolScope)bcar).getSymbol();
				}
