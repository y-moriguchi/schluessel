 ...
 */
public class LispLabel extends LightweightGUIElement
implements ILispComponent, HasText {
	...
	public static class MakeLabel extends UnaryArgs {
		...
		protected Datum execute(
			...
			if(c1a instanceof LispString) {
				return new LispLabel(new JLabel(c1a.getString()));
			} else {
	 ...
	 */
	public static class LabelIconSetS extends BinaryArgs {
		...
		protected Datum execute(Datum c1a, Datum c2a, Environment env,
				LispMessage mesg) {
			if(!(c2a instanceof ILispIcon)) {
				throw mesg.getError("err.swing.require.icon", c2a);
			} else if(c1a instanceof LispLabel) {
				((LispLabel)c1a).label.setIcon(
						((ILispIcon)c2a).getIcon());
				return Undef.UNDEF;
			} else {
				throw mesg.getError("err.swing.require.label", c1a);
			}
		}
