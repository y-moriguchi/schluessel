 ...
 */
public class JtableRowInsertS extends TernaryArgs {
		...
		if(c1a instanceof ILispTable) {
			if(r < 0) {
				throw mesg.getError("err.require.int.nonnegative",
						c2a);
			} else if(r > ((ILispTable)c1a).getRowCount()) {
				throw mesg.getError("err.table.index.outofrange",
						c2a);
			}

			try {
				((ILispTable)c1a).insertRow(r, c3a);
				return Undef.UNDEF;
			} catch(UnsupportedOperationException e) {
				throw mesg.getError("err.table.immutable", c1a);
			}
		} else {
