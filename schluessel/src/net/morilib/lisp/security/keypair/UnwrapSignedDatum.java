 ...
 */
public class UnwrapSignedDatum extends UnaryArgs {
	...
	protected Datum execute(Datum c1a, Environment env,
			LispMessage mesg) {
		if(c1a instanceof LispSignedDatum) {
			try {
				return LispJavaUtils.newInstance(
						((LispSignedDatum)c1a).datum.getObject());
			} catch (IOException e) {
				throw mesg.getError("err.io");
			} catch (ClassNotFoundException e) {
				throw mesg.getError("err.keypair.cannotunwrap");
			}
		} else {
