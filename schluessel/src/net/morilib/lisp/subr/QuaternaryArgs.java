 ...
 */
public abstract class QuaternaryArgs extends Subr {

	/**
	 * 
	 */
	public QuaternaryArgs() {
		// do nothing
	}
	 ...
	 * @param name
	 */
	public QuaternaryArgs(String name) {
		symbolName = name;
	}
	...
	protected abstract Datum execute(
			Datum c1a, Datum c2a, Datum c3a, Datum c4a,
			...
			if(c1.getCdr() instanceof Cons) {
				...
				if(c2.getCdr() instanceof Cons) {
					...
					if(c3.getCdr() instanceof Cons) {
						...
						if(c4.getCdr() == Nil.NIL) {
							return execute(c1a, c2a, c3a, c4a, env,
									mesg);
						}
					}
				}
