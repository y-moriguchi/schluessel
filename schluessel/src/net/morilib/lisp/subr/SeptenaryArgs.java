 ...
 */
public abstract class SeptenaryArgs extends Subr {

	/**
	 * 
	 */
	public SeptenaryArgs() {
		// do nothing
	}
	 ...
	 * @param name
	 */
	public SeptenaryArgs(String name) {
		symbolName = name;
	}
	...
	protected abstract Datum execute(
			...
			Datum c6a, Datum c7a, Environment env, LispMessage mesg);
					...
					if(c3.getCdr() instanceof Cons) {
						...
						if(c4.getCdr() instanceof Cons) {
							...
							if(c5.getCdr() instanceof Cons) {
								...
								if(c6.getCdr() instanceof Cons) {
									Cons c7 = (Cons)c6.getCdr();
									Datum c7a = c6.getCar();

									if(c7.getCdr() == Nil.NIL) {
										return execute(c1a, c2a, c3a,
												c4a, c5a, c6a, c7a,
												env, mesg);
									}
								}
