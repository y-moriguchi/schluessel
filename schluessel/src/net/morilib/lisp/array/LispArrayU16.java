 ...
 */
public class LispArrayU16 extends SRFI47Array
implements java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/05/23
	 */
	public static class Au16 extends Subr {
	...
	implements LispArrayPrototype {
		...
		public ILispArray makeArray(int... is) {
			return new LispArrayU16(real, is);
		}
	...
	public LispVector toVector() {
		...
		for(int i = 0; i < r.length; i++) {
			r[i] = LispInteger.valueOf(Bytes.ushortToInt(real[i]));
		}
	...
	public Datum getFromArray(int... is) {
		return LispInteger.valueOf(Bytes.ushortToInt(
	...
	public void setToArray(Datum d, int... is) {
		...
		} else if(!((LispReal)d).inUnsignedShortRange()) {
	...
	public boolean isEqualTo(ILispArray a) {
		if(!isIndexEqualTo(a)) {
			return false;
		} else if(a instanceof LispArrayU16) {
			return Arrays.equals(real, ((LispArrayU16)a).real);
		} else {
	...
	public void fill(Iterator<Datum> itr) {
		for(int i = 0; i < real.length && itr.hasNext(); i++) {
			...
			if(!(x instanceof LispReal)) {
				throw new ClassCastException();
			} else if(!((LispReal)x).inUnsignedShortRange()) {
