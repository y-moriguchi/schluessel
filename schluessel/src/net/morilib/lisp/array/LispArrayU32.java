 ...
 */
public class LispArrayU32 extends SRFI47Array
implements java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/05/23
	 */
	public static class Au32 extends Subr {
		...
		public Datum eval(Datum body, Environment env,
			...
			if(d == null || d instanceof LispReal) {
				return new Prototype((d == null) ? 0 : d.getInt());
			} else {
	...
	implements LispArrayPrototype {
		...
		public ILispArray makeArray(int... is) {
			return new LispArrayU32(real, is);
		}
	 ...
	 * @param is
	 */
	public LispArrayU32(int r, int... is) {
		super(is);
		real     = new int[LispArrayShape.arraylength(is)];
		Arrays.fill(real, r);
	}
	...
	public LispVector toVector() {
		...
		for(int i = 0; i < r.length; i++) {
			r[i] = LispInteger.valueOf(Bytes.uintToLong(real[i]));
		}
	...
	public Datum getFromArray(int... is) {
		return LispInteger.valueOf(Bytes.uintToLong(
	...
	public boolean isEqualTo(ILispArray a) {
		if(!isIndexEqualTo(a)) {
			return false;
		} else if(a instanceof LispArrayU32) {
			return Arrays.equals(real, ((LispArrayU32)a).real);
		} else {
	...
	public void fill(Iterator<Datum> itr) {
		for(int i = 0; i < real.length && itr.hasNext(); i++) {
			...
			if(!(x instanceof LispReal)) {
				throw new ClassCastException();
			} else if(!((LispReal)x).inUnsignedIntRange()) {
