 ...
 */
public class LispArrayS16 extends SRFI47Array
implements java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/05/23
	 */
	public static class As16 extends Subr {
	...
	implements LispArrayPrototype {
		...
		public ILispArray makeArray(int... is) {
			return new LispArrayS16(real, is);
		}
	 ...
	 * @param is
	 */
	public LispArrayS16(short r, int... is) {
		super(is);
		real     = new short[LispArrayShape.arraylength(is)];
		Arrays.fill(real, r);
	}
	...
	public boolean isEqualTo(ILispArray a) {
		if(!isIndexEqualTo(a)) {
			return false;
		} else if(a instanceof LispArrayS16) {
			return Arrays.equals(real, ((LispArrayS16)a).real);
		} else {
	...
	public void fill(Iterator<Datum> itr) {
		for(int i = 0; i < real.length && itr.hasNext(); i++) {
			...
			if(!(x instanceof LispReal)) {
				throw new ClassCastException();
			} else if(!((LispReal)x).inShortRange()) {
				throw new ValueOutOfBoundsException(x.toString());
			} else {
				real[i] = (short)x.getInt();
			}
		}
	}
