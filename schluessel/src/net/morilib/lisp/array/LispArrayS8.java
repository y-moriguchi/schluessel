...
implements java.io.Serializable {
	...
	implements LispArrayPrototype {
		...
		public ILispArray makeArray(int... is) {
			return new LispArrayS8(real, is);
		}
	 ...
	 * @param is
	 */
	public LispArrayS8(byte r, int... is) {
		super(is);
	...
	public void setToArray(Datum d, int... is) {
		...
		} else if(!((LispReal)d).inByteRange()) {
	...
	public boolean isEqualTo(ILispArray a) {
		if(!isIndexEqualTo(a)) {
			return false;
		} else if(a instanceof LispArrayS8) {
			return Arrays.equals(real, ((LispArrayS8)a).real);
		} else {
	...
	public void fill(Iterator<Datum> itr) {
		for(int i = 0; i < real.length && itr.hasNext(); i++) {
			...
			if(!(x instanceof LispReal)) {
				throw new ClassCastException();
			} else if(!((LispReal)x).inByteRange()) {
