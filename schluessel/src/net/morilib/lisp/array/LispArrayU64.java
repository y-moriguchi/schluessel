 ...
 */
public class LispArrayU64 extends SRFI47Array
implements java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/05/23
	 */
	public static class Au64 extends Subr {
		...
		public Datum eval(Datum body, Environment env,
			...
			if(d == null || d instanceof LispReal) {
				return new Prototype((d == null) ? 0 : d.getLong());
			} else {
	...
	implements LispArrayPrototype {
		...
		public ILispArray makeArray(int... is) {
			return new LispArrayU64(real, is);
		}
	 ...
	 * @param is
	 */
	public LispArrayU64(long r, int... is) {
		super(is);
		real     = new long[LispArrayShape.arraylength(is)];
		Arrays.fill(real, r);
	}
	...
	public LispVector toVector() {
		...
		for(int i = 0; i < r.length; i++) {
			r[i] = LispInteger.valueOf(Bytes.ulongToBigInteger(
					real[i]));
		}
	...
	public Datum getFromArray(int... is) {
		return LispInteger.valueOf(Bytes.ulongToBigInteger(
	...
	public boolean isEqualTo(ILispArray a) {
		if(!isIndexEqualTo(a)) {
			return false;
		} else if(a instanceof LispArrayU64) {
			return Arrays.equals(real, ((LispArrayU64)a).real);
		} else {
	...
	public void fill(Iterator<Datum> itr) {
		for(int i = 0; i < real.length && itr.hasNext(); i++) {
			...
			if(!(x instanceof LispReal)) {
				throw new ClassCastException();
			} else if(!((LispReal)x).inUnsignedLongRange()) {
