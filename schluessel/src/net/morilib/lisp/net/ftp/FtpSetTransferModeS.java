 ...
 */
public class FtpSetTransferModeS extends BinaryArgs {
	...
	protected Datum execute(Datum c1a, Datum c2a, Environment env,
		...
		if(c1a instanceof LispFTPConnection) {
			if(c2a instanceof LispFTPTransferMode) {
				throw mesg.getError("err.net.require.ftptransfermode",
						c2a);
			}

			fn = ((LispFTPTransferMode)c2a).mode;
			try {
				((LispFTPConnection)c1a).client.setMode(fn);
				return c2a;
			} catch (IOException e) {
