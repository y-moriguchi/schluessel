 ...
 */
public class BlobCopy extends UnaryArgs {
	...
	protected Datum execute(Datum c1a, Environment env,
		...
		if(c1a instanceof LispBlob) {
			...
			for(int i = 0; i < ((LispBlob)c1a).length(); i++) {
				v.addByte(((LispBlob)c1a).get(i));
			}
			return new LispBlob(ByteBuffer.wrap(v.toByteArray()));
		} else {
