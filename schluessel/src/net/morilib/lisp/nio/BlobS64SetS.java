 ...
 */
public class BlobS64SetS extends QuaternaryArgs {

	//
	private static long getNumber(Datum d, LispMessage mesg) {
		if(!(d instanceof LispInteger)) {
			throw mesg.getError("err.require.real", d);
		} else if(((LispInteger)d).inLongRange()) {
			return (long)d.getLong();
		} else {
	...
	protected Datum execute(Datum c1a, Datum c2a, Datum c3a, Datum c4a,
		...
		if(!(c1a instanceof SRFI74Endianness.Endian)) {
			...
			try {
				((LispBlob)c2a).order((SRFI74Endianness.Endian)c2a);
				((LispBlob)c2a).putLong(k, getNumber(c4a, mesg));
				return Undef.UNDEF;
