...
public class SubrCallWithValues extends Subr {
	...
	/*package*/ ClosureClass createClosureClass(Environment env) {
		...
		bld.addReferSymbol(Symbol.getSymbol("consume"));
		bld.addReferSymbol(Symbol.getSymbol("produce"));
		bld.addPush(Nil.NIL);
		bld.addCall();   // producer
		bld.addAppendValues();
		bld.addCall();   // consumer
		bld.addReturnOp();
		
		List<Datum> prm = new ArrayList<Datum>();
		prm.add(Symbol.getSymbol("produce"));
		prm.add(Symbol.getSymbol("consume"));
