 ...
 */
public class LispF32Vector extends Atom
implements HomogeneousFloatArray, java.io.Serializable {
	...
	public static class F32Vector extends Subr {
		...
		public Datum eval(
			...
			return new LispF32Vector(toFloatList(body, mesg));
		}
	...
	public static class MakeF32Vector extends Subr {
		...
		private float[] makeVector(int len, double d) {
			FloatVector b = new FloatArrayVector();
		...
		public Datum eval(
			...
			if(lst.size() == 1) {
				...
				return new LispF32Vector(makeVector(t, 0.0));
			} else if(lst.size() == 2) {
				int t = SubrUtils.getSmallInt(
						lst.get(0), mesg);
				...
				if(t < 0) {
					...
					throw mesg.getError(
							"err.uvector.outofrange.f32", lst.get(1));
				}
				return new LispF32Vector(
	...
	public static class ListToF32Vector extends UnaryArgs {
		...
		protected Datum execute(
			...
			if(c1a instanceof Cons || c1a.equals(Nil.NIL)) {
				return new LispF32Vector(toFloatList(c1a, mesg));
			} else {
	...
	public static class EqualF32Vector extends BinaryArgs {
		...
		protected Datum execute(
			...
			return LispBoolean.getInstance(
					((LispF32Vector)c1a).vector.equals(
							((LispF32Vector)c2a).vector));
		}
	...
	private static float[] toFloatList(Datum body, LispMessage mesg) {
		...
		FloatVector v = new FloatArrayVector();
	...
	public boolean equalsArray(Datum d1, Datum d2) {
		if(!(d1 instanceof LispF32Vector)) {
			return false;
		} else if(!(d2 instanceof LispF32Vector)) {
			return false;
		}
		return ((LispF32Vector)d1).vector.equals(
				((LispF32Vector)d2).vector);
	}
	...
	public LispFloat get(int index) {
		return new LispFloat(vector.getFloat(index));
	}
