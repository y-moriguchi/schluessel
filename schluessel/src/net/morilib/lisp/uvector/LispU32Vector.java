 ...
 */
public class LispU32Vector extends Atom
implements HomogeneousUnsignedArray, java.io.Serializable {
	...
	public static class U32Vector extends Subr {
		...
		public Datum eval(
			...
			return new LispU32Vector(toIntList(body, mesg));
		}
	...
	public static class MakeU32Vector extends Subr {
		...
		public Datum eval(
			...
			if(lst.size() == 1) {
				...
				return new LispU32Vector(makeVector(t, 0));
			} else if(lst.size() == 2) {
				int t = SubrUtils.getSmallInt(
						lst.get(0), mesg);
				...
				if(t < 0) {
					...
					throw mesg.getError(
							"err.uvector.outofrange.u32", lst.get(1));
				}
				return new LispU32Vector(makeVector(
						t, lst.get(1).getLong()));
			} else {
			...
			if(c1a instanceof LispU32Vector) {
				...
				for(int i = 0; i < v.vector.size(); i++) {
					b.append(LispInteger.valueOf(
							Bytes.uintToLong(v.vector.getInt(i))));
				}
				return b.get();
			} else {
	...
	public static class ListToU32Vector extends UnaryArgs {
		...
		protected Datum execute(
			...
			if(c1a instanceof Cons || c1a.equals(Nil.NIL)) {
				return new LispU32Vector(toIntList(c1a, mesg));
			} else {
	...
	public static class EqualU32Vector extends BinaryArgs {
		...
		protected Datum execute(
			...
			} else if(!(c2a instanceof LispU32Vector)) {
				throw mesg.getError("err.uvector.require.u32", c2a);
			}
			return LispBoolean.getInstance(
					((LispU32Vector)c1a).vector.equals(
							((LispU32Vector)c2a).vector));
		}
	...
	public String print() {
		...
		for(int i = 0; i < vector.size(); i++) {
			b.append(d);
			b.append(Bytes.uintToLong(vector.getInt(i)));
			d = " ";
		}
		b.append(")");
		return b.toString();
	...
	public boolean equalsArray(Datum d1, Datum d2) {
		if(!(d1 instanceof LispU32Vector)) {
			return false;
		} else if(!(d2 instanceof LispU32Vector)) {
			return false;
		}
		return ((LispU32Vector)d1).vector.equals(
				((LispU32Vector)d2).vector);
	}
	...
	public LispReal get(int index) {
		return LispInteger.valueOf(
				Bytes.uintToLong(vector.getInt(index)));
	}
	...
	public void set(int index, LispReal x) {
		vector.setInt(index, (int)x.getLong());
	}
