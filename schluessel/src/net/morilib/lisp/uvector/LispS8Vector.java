 ...
 */
public class LispS8Vector extends Atom
implements HomogeneousSignedArray, ILispBytevector,
java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class S8Vector extends Subr {
		...
		public Datum eval(
			...
			return new LispS8Vector(toByteList(body, mesg));
		}
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class IsS8Vector extends UnaryArgs {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class MakeS8Vector extends Subr {
		...
		public Datum eval(
			...
			if(lst.size() == 1) {
				...
				return new LispS8Vector(makeVector(t, 0));
			} else if(lst.size() == 2) {
				int t = SubrUtils.getSmallInt(
						lst.get(0), mesg);
				...
				if(t < 0) {
					...
					throw mesg.getError(
							"err.uvector.outofrange.s8", lst.get(1));
				}
				return new LispS8Vector(
						makeVector(t, lst.get(1).getInt()));
			} else {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class S8VectorToList extends UnaryArgs {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class ListToS8Vector extends UnaryArgs {
		...
		protected Datum execute(
			...
			if(c1a instanceof Cons || c1a.equals(Nil.NIL)) {
				return new LispS8Vector(toByteList(c1a, mesg));
			} else {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class EqualS8Vector extends BinaryArgs {
		...
		protected Datum execute(
			...
			} else if(!(c2a instanceof LispS8Vector)) {
				throw mesg.getError("err.uvector.require.s8", c2a);
			}
			return LispBoolean.getInstance(
					((LispS8Vector)c1a).vector.equals(
							((LispS8Vector)c2a).vector));
		}
	...
	public boolean equalsArray(Datum d1, Datum d2) {
		if(!(d1 instanceof LispS8Vector)) {
			return false;
		} else if(!(d2 instanceof LispS8Vector)) {
			return false;
		}
		return ((LispS8Vector)d1).vector.equals(
				((LispS8Vector)d2).vector);
	}
	...
	public LispInteger get(int index) {
		return LispInteger.valueOf(vector.getInt(index));
	}
	...
	public void set(int index, LispReal x) {
		vector.setByte(index, x.castByte());
	}
