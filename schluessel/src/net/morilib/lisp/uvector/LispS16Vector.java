 ...
 */
public class LispS16Vector extends Atom
implements HomogeneousSignedArray, java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class S16Vector extends Subr {
		...
		public Datum eval(
			...
			return new LispS16Vector(toShortList(body, mesg));
		}
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class IsS16Vector extends UnaryArgs {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class MakeS16Vector extends Subr {
		...
		public Datum eval(
			...
			if(lst.size() == 1) {
				...
				return new LispS16Vector(makeVector(t, 0));
			} else if(lst.size() == 2) {
				int t = SubrUtils.getSmallInt(
						lst.get(0), mesg);
				...
				if(t < 0) {
					...
					throw mesg.getError(
							"err.uvector.outofrange.s16", lst.get(1));
				}
				return new LispS16Vector(
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class S16VectorToList extends UnaryArgs {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class EqualS16Vector extends BinaryArgs {
		...
		protected Datum execute(
			...
			} else if(!(c2a instanceof LispS16Vector)) {
				throw mesg.getError("err.uvector.require.s16", c2a);
			}
			return LispBoolean.getInstance(
					((LispS16Vector)c1a).vector.equals(
							((LispS16Vector)c2a).vector));
		}
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class ListToS16Vector extends UnaryArgs {
		...
		protected Datum execute(
			...
			if(c1a instanceof Cons || c1a.equals(Nil.NIL)) {
				return new LispS16Vector(toShortList(c1a, mesg));
			} else {
	...
	public String print() {
		...
		for(int i = 0; i < vector.size(); i++) {
			b.append(d);
			b.append((int)vector.getShort(i));
			d = " ";
		}
		b.append(")");
		return b.toString();
	...
	public boolean equalsArray(Datum d1, Datum d2) {
		if(!(d1 instanceof LispS16Vector)) {
			return false;
		} else if(!(d2 instanceof LispS16Vector)) {
			return false;
		}
		return ((LispS16Vector)d1).vector.equals(
				((LispS16Vector)d2).vector);
	}
	...
	public void set(int index, LispReal x) {
		vector.setShort(index, x.castShort());
	}
