 ...
 */
public class LispS32Vector extends Atom
implements HomogeneousSignedArray, java.io.Serializable {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class S32Vector extends Subr {
		...
		public Datum eval(
			...
			return new LispS32Vector(toIntList(body, mesg));
		}
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class IsS32Vector extends UnaryArgs {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class MakeS32Vector extends Subr {

		//
		private int[] makeVector(int len, int d) {
			IntegerVector b = new IntegerArrayVector();
		...
		public Datum eval(
			...
			if(lst.size() == 1) {
				...
				return new LispS32Vector(makeVector(t, 0));
			} else if(lst.size() == 2) {
				int t = SubrUtils.getSmallInt(
						lst.get(0), mesg);
				...
				if(t < 0) {
					...
					throw mesg.getError(
							"err.uvector.outofrange.s32", lst.get(1));
				}
				return new LispS32Vector(
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class S32VectorToList extends UnaryArgs {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class ListToS32Vector extends UnaryArgs {
		...
		protected Datum execute(
			...
			if(c1a instanceof Cons || c1a.equals(Nil.NIL)) {
				return new LispS32Vector(toIntList(c1a, mesg));
			} else {
	 ...
	 * @author MORIGUCHI, Yuichiro 2011/03/06
	 */
	public static class EqualS32Vector extends BinaryArgs {
		...
		protected Datum execute(
			...
			return LispBoolean.getInstance(
					((LispS32Vector)c1a).vector.equals(
							((LispS32Vector)c2a).vector));
		}
	...
	public boolean equalsArray(Datum d1, Datum d2) {
		if(!(d1 instanceof LispS32Vector)) {
			return false;
		} else if(!(d2 instanceof LispS32Vector)) {
			return false;
		}
		return ((LispS32Vector)d1).vector.equals(
				((LispS32Vector)d2).vector);
	}
