 ...
 */
public class GetBytevectorSome extends UnaryArgs {
	...
	protected Datum execute(Datum c1a, Environment env,
		...
		if(c1a instanceof ILispBinaryInputPort) {
			try {
				byte[] b = ((ILispBinaryInputPort)c1a).getBytesSome();

				return (b != null) ?
						new LispU8Vector(b) : EOFObject.EOF;
			} catch (IOException e) {
				throw mesg.getError("err.io", e.getMessage());
			}
		} else {
			throw mesg.getError("err.io.require.port.binary.input",
					c1a);
		}
	}
