 ...
 */
public class OpenFileOutputPort extends Subr {
	...
	//		String fn = SubrUtils.nextString(itr, mesg, body);
			...
			if(!(op instanceof LispFileOptions)) {
				...
				otr = ((ILispTranscoder)tr).newOutput(
						((LispFileOptions)op).openForOutput(fn, mesg));
				otr = ((LispBufferMode)bm).getTranscoder(otr);
				return new LispTextualOutputPort(
						otr, (ILispTranscoder)tr, (LispBufferMode)bm);
			} else {
				ous = ((LispFileOptions)op).openForOutput(fn, mesg);
