 ...
 */
public class PutString extends Subr {
	...
	public Datum eval(Datum body, Environment env, LispMessage mesg) {
		...
		if(!(d1 instanceof ILispTextualOutputPort)) {
			throw mesg.getError("err.io.require.port.binary.output",
					d1);
		} else if(d2 instanceof LispString) {
			b  = (LispString)d2;
			...
			try {
				((ILispTextualOutputPort)d1).putString(
						b.getString(), st, en);
				return Undef.UNDEF;
			} catch (IOException e) {
			...
			throw mesg.getError("err.require.string", d2);
		}
	}
