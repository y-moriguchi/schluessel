 ...
 */
public class BytevectorToString extends BinaryArgs {
	...
	protected Datum execute(Datum c1a, Datum c2a, Environment env,
		...
		InputTranscoder tri;
		StringOutputTranscoder tro;

		if(!(c1a instanceof ILispBytevector)) {
			throw mesg.getError("err.uvector.require.8", c1a);
		} else if(!(c2a instanceof ILispTranscoder)) {
			throw mesg.getError("err.io.require.transcoder", c2a);
		} else {
			try {
				ins = new ByteArrayInputStream(
						((ILispBytevector)c1a).toBytes());
				tri = ((ILispTranscoder)c2a).newInput(ins);
				tro = new StringOutputTranscoder();
