 ...
 */
public class BytevectorFillS extends Subr {
	...
	public Datum eval(Datum body, Environment env, LispMessage mesg) {
		...
		f = LispBytevector.datumToByteOrOctet(d2, mesg);
		s = (d3 == null) ? 0 : SubrUtils.getSmallInt(d3, mesg);
		e = (d4 == null) ?
				v.length() : SubrUtils.getSmallInt(d4, mesg);
		if(s < 0 || s >= v.length()) {
			throw mesg.getError("err.range.invalid", d3);
		} else if(e < 0 || e > v.length()) {
			throw mesg.getError("err.range.invalid", d4);
		} else if(e < s) {
