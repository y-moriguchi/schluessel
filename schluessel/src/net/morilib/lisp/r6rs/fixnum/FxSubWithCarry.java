 ...
 */
public class FxSubWithCarry extends AbstractFixnumTernaryOp3 {
	...
	protected int op1(int x, int y, int z, LispMessage mesg) {
		long r = (long)x - (long)y - (long)z;

		if(x >= 0 && y <= 0 && z <= 0 && (int)r < 0) {
			// overflowed
			return (int)(r - LispInteger.FIXNUM_MIN_LONG);
		} else if(x <= 0 && y >= 0 && z >= 0 && (int)r > 0) {
			// overflowed
			return (int)(r + LispInteger.FIXNUM_MIN_LONG);
		} else if(r == LispInteger.FIXNUM_MIN_LONG) {
			return 0;
		} else {
			return (int)r;
		}
