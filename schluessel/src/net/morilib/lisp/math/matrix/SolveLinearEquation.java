 ...
 */
public class SolveLinearEquation extends BinaryArgs {
	...
	protected Datum execute(Datum c1a, Datum c2a, Environment env,
		...
		if(!(c1a instanceof ILispMatrix)) {
			throw mesg.getError("err.matrix.require.numbermatrix",
					c1a);
		} else if(!(c2a instanceof ILispNumberVector)) {
			throw mesg.getError("err.matrix.require.numbervector",
					c1a);
		} else {
			try {
				return (Datum)LispMatrices.solve((ILispMatrix)c1a,
						(ILispNumberVector)c2a);
