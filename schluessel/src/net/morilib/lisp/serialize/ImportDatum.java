 ...
 */
public class ImportDatum extends UnaryArgs {
	...
	protected Datum execute(Datum c1a, Environment env,
		...
		try {
			s   = LispFiles.getFile(env, c1a, mesg);
			pbs = new PushbackInputStream(new FileInputStream(s));
			if(!IOs.isStreamSerialized(pbs, pbb)) {
				throw mesg.getError("err.serialize.file.notserialized",
						s.toString());
			}
