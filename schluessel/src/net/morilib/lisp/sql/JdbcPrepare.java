...
public class JdbcPrepare extends BinaryArgs {
	...
	protected Datum execute(
			...
			LispMessage mesg) {
		if(!(c1a instanceof LispJdbcConnection)) {
			throw mesg.getError("err.jdbc.require.connection", c1a);
		} else if(!(c2a instanceof LispString)) {
			throw mesg.getError("err.require.string", c2a);
		} else {
			try {
				return new LispJdbcPrepared(
						((LispJdbcConnection)c1a).getConnection(),
						c2a.getString());
