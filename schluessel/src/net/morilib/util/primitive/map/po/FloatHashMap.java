...
implements java.io.Serializable {
	...
	private static class Ent<V> implements PrimitiveEntryK<V> {
		...
		//
		@SuppressWarnings({ "rawtypes", "unchecked" })
		private Object putEntry(float k, V v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatHashMap(FloatMap<V> mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}

	//
	private Iterator<PrimitiveEntryK<V>> floatEntrySetIterator() {
	 ...
	 */
	public Set<FloatMap.PrimitiveEntryK<V>> floatKeyEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntryK<V>>() {

				public Iterator<FloatMap.PrimitiveEntryK<V>> iterator() {
					return floatEntrySetIterator();
				}

				public int size() {
					return FloatHashMap.this.size();
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.po.AbstractFloatMap#getElement(float)
	 */
	@SuppressWarnings("unchecked")
	...
	//
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private boolean _putEntry(float k, V v) {
		int h = getHash(k) % ent.length;
