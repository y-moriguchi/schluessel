...
implements java.io.Serializable {
	...
	private static class Ent<V> implements PrimitiveEntryK<V> {
		...
		//
		@SuppressWarnings({ "rawtypes", "unchecked" })
		private Object putEntry(long k, V v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public LongHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public LongHashMap(LongMap<V> mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}

	//
	private Iterator<PrimitiveEntryK<V>> longEntrySetIterator() {
	 ...
	 */
	public Set<LongMap.PrimitiveEntryK<V>> longKeyEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntryK<V>>() {

				public Iterator<LongMap.PrimitiveEntryK<V>> iterator() {
					return longEntrySetIterator();
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.po.AbstractLongMap#getElement(long)
	 */
	@SuppressWarnings("unchecked")
	...
	//
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private boolean _putEntry(long k, V v) {
		int h = getHash(k) % ent.length;
