...
implements java.io.Serializable {
	...
	private static class Ent<V> implements PrimitiveEntryK<V> {
		...
		//
		@SuppressWarnings({ "rawtypes", "unchecked" })
		private Object putEntry(short k, V v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ShortHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ShortHashMap(ShortMap<V> mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	...
	//
	private Iterator<PrimitiveEntryK<V>> shortEntrySetIterator() {
	 ...
	 */
	public Set<ShortMap.PrimitiveEntryK<V>> shortKeyEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntryK<V>>() {

				public Iterator<ShortMap.PrimitiveEntryK<V>> iterator() {
					return shortEntrySetIterator();
				}

				public int size() {
					return ShortHashMap.this.size();
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.po.AbstractShortMap#getElement(short)
	 */
	@SuppressWarnings("unchecked")
	...
	public V putElement(short k, V v) {
		int h = getHash(k) % ent.length;
	...
	//
	@SuppressWarnings({ "rawtypes", "unchecked" })
	private boolean _putEntry(short k, V v) {
		int h = getHash(k) % ent.length;
