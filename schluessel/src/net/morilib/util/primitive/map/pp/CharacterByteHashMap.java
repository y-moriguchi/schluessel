 ...
 */
public class CharacterByteHashMap
extends AbstractCharacterByteMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(char key, byte value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(char k, Byte v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public CharacterByteHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public CharacterByteHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public CharacterByteHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public CharacterByteHashMap(CharacterByteMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<CharacterByteMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return CharacterByteHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(char k, Byte v) {
		int h = getHash(k) % ent.length;
