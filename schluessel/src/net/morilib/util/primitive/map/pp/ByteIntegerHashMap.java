 ...
 */
public class ByteIntegerHashMap
extends AbstractByteIntegerMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(byte key, int value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(byte k, Integer v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ByteIntegerHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public ByteIntegerHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public ByteIntegerHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ByteIntegerHashMap(ByteIntegerMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<ByteIntegerMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return ByteIntegerHashMap.this.size();
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.ByteIntegerMap#getElement(byte)
	 */
	public int f(byte k) {
		return _get(k).value;
	}
	...
	//
	private boolean _putEntry(byte k, Integer v) {
		int h = getHash(k) % ent.length;
