...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(float key, long value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(float k, Long v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatLongHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public FloatLongHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatLongHashMap(FloatLongMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<FloatLongMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.FloatLongMap#getElement(float)
	 */
	...
	//
	private boolean _putEntry(float k, Long v) {
		int h = getHash(k) % ent.length;
