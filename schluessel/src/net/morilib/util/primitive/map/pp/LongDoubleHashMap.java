...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(long key, double value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(long k, Double v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public LongDoubleHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public LongDoubleHashMap(LongDoubleMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<LongDoubleMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return LongDoubleHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(long k, Double v) {
		int h = getHash(k) % ent.length;
