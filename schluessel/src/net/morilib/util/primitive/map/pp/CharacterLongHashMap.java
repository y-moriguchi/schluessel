 ...
 */
public class CharacterLongHashMap
extends AbstractCharacterLongMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(char key, long value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(char k, Long v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public CharacterLongHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public CharacterLongHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public CharacterLongHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public CharacterLongHashMap(CharacterLongMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<CharacterLongMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return CharacterLongHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(char k, Long v) {
		int h = getHash(k) % ent.length;
	...
	public Long removeElement(char k) {
		...
		return (r != null) ? (Long)r.value : null;
	}
