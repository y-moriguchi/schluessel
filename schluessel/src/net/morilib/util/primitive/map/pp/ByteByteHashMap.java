...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(byte key, byte value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(byte k, Byte v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ByteByteHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public ByteByteHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ByteByteHashMap(ByteByteMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<ByteByteMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return ByteByteHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(byte k, Byte v) {
		int h = getHash(k) % ent.length;
