...
extends AbstractCharacterIntegerMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(char key, int value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(char k, Integer v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public CharacterIntegerHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public CharacterIntegerHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public CharacterIntegerHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public CharacterIntegerHashMap(CharacterIntegerMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<CharacterIntegerMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return CharacterIntegerHashMap.this.size();
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.CharacterIntegerMap#getElement(char)
	 */
	public int f(char k) {
		return _get(k).value;
	}
	 ...
	 */
	public Integer putElement(char k, int v) {
		int h = getHash(k) % ent.length;
	...
	//
	private boolean _putEntry(char k, Integer v) {
		int h = getHash(k) % ent.length;
	 ...
	 */
	public Integer removeElement(char k) {
		Ent r = _remove(k);
