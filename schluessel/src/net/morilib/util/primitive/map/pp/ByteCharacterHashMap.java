 ...
 */
public class ByteCharacterHashMap
extends AbstractByteCharacterMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(byte key, char value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(byte k, Character v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ByteCharacterHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public ByteCharacterHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public ByteCharacterHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ByteCharacterHashMap(ByteCharacterMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<ByteCharacterMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return ByteCharacterHashMap.this.size();
				}
	 ...
	 */
	public Character putElement(byte k, char v) {
		int h = getHash(k) % ent.length;
	...
	//
	private boolean _putEntry(byte k, Character v) {
		int h = getHash(k) % ent.length;
