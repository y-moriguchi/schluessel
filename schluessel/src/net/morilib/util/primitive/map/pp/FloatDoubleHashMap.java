...
extends AbstractFloatDoubleMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(float key, double value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(float k, Double v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatDoubleHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public FloatDoubleHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatDoubleHashMap(FloatDoubleMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<FloatDoubleMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.FloatDoubleMap#getElement(float)
	 */
	...
	//
	private boolean _putEntry(float k, Double v) {
		int h = getHash(k) % ent.length;
