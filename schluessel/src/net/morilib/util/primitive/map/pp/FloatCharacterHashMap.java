 ...
 */
public class FloatCharacterHashMap
extends AbstractFloatCharacterMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(float key, char value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(float k, Character v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatCharacterHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public FloatCharacterHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public FloatCharacterHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatCharacterHashMap(FloatCharacterMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<FloatCharacterMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return FloatCharacterHashMap.this.size();
				}
	 ...
	 */
	public Character putElement(float k, char v) {
		int h = getHash(k) % ent.length;
	...
	//
	private boolean _putEntry(float k, Character v) {
		int h = getHash(k) % ent.length;
