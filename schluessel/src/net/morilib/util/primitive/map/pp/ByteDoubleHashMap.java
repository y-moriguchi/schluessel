...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(byte key, double value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(byte k, Double v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ByteDoubleHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public ByteDoubleHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ByteDoubleHashMap(ByteDoubleMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<ByteDoubleMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.ByteDoubleMap#getElement(byte)
	 */
	...
	//
	private boolean _putEntry(byte k, Double v) {
		int h = getHash(k) % ent.length;
