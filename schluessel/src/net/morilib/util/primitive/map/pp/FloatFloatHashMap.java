...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(float key, float value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(float k, Float v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatFloatHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatFloatHashMap(FloatFloatMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<FloatFloatMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.FloatFloatMap#getElement(float)
	 */
	...
	//
	private boolean _putEntry(float k, Float v) {
		int h = getHash(k) % ent.length;
