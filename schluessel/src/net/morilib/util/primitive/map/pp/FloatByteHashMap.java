...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(float key, byte value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(float k, Byte v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatByteHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public FloatByteHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatByteHashMap(FloatByteMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<FloatByteMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return FloatByteHashMap.this.size();
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.FloatByteMap#getElement(float)
	 */
	...
	//
	private boolean _putEntry(float k, Byte v) {
		int h = getHash(k) % ent.length;
	...
	public Byte removeElement(float k) {
		...
		return (r != null) ? (Byte)r.value : null;
	}
