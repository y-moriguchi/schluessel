...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(double key, long value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(double k, Long v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public DoubleLongHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public DoubleLongHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public DoubleLongHashMap(DoubleLongMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<DoubleLongMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	//
	private boolean _putEntry(double k, Long v) {
		int h = getHash(k) % ent.length;
