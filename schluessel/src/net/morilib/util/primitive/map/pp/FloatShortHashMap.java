...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(float key, short value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(float k, Short v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public FloatShortHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public FloatShortHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public FloatShortHashMap(FloatShortMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<FloatShortMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return FloatShortHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(float k, Short v) {
		int h = getHash(k) % ent.length;
