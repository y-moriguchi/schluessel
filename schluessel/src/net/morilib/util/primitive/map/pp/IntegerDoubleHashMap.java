 ...
 */
public class IntegerDoubleHashMap
extends AbstractIntegerDoubleMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(int key, double value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(int k, Double v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public IntegerDoubleHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public IntegerDoubleHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public IntegerDoubleHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public IntegerDoubleHashMap(IntegerDoubleMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<IntegerDoubleMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return IntegerDoubleHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(int k, Double v) {
		int h = getHash(k) % ent.length;
