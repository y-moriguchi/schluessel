 ...
 */
public class CharacterCharacterHashMap
extends AbstractCharacterCharacterMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(char key, char value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(char k, Character v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public CharacterCharacterHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param initialCapacity
	 */
	public CharacterCharacterHashMap(int initialCapacity) {
	 ...
	 * @param loadFactor
	 */
	public CharacterCharacterHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public CharacterCharacterHashMap(CharacterCharacterMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<CharacterCharacterMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
			entrySet = new AbstractSet<PrimitiveEntry>() {
				...
				public int size() {
					return CharacterCharacterHashMap.this.size();
				}
	...
	//
	private boolean _putEntry(char k, Character v) {
		int h = getHash(k) % ent.length;
