...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(long key, byte value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(long k, Byte v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public LongByteHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public LongByteHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public LongByteHashMap(LongByteMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<LongByteMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.LongByteMap#getElement(long)
	 */
	...
	//
	private boolean _putEntry(long k, Byte v) {
		int h = getHash(k) % ent.length;
