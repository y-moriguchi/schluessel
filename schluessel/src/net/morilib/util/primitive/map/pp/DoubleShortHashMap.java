...
extends AbstractDoubleShortMap
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(double key, short value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(double k, Short v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public DoubleShortHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public DoubleShortHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public DoubleShortHashMap(DoubleShortMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<DoubleShortMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	//
	private boolean _putEntry(double k, Short v) {
		int h = getHash(k) % ent.length;
