...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(byte key, float value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(byte k, Float v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ByteFloatHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public ByteFloatHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ByteFloatHashMap(ByteFloatMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<ByteFloatMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	//
	private boolean _putEntry(byte k, Float v) {
		int h = getHash(k) % ent.length;
