...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(byte key, short value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(byte k, Short v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public ByteShortHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public ByteShortHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public ByteShortHashMap(ByteShortMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<ByteShortMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.pp.ByteShortMap#getElement(byte)
	 */
	...
	//
	private boolean _putEntry(byte k, Short v) {
		int h = getHash(k) % ent.length;
