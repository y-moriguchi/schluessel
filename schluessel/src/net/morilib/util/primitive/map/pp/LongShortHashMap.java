...
implements java.io.Serializable {
	...
	private static class Ent implements PrimitiveEntry {
		...
		//
		private Ent(long key, short value) {
			this.key   = key;
			this.value = value;
		}
		...
		//
		private Object putEntry(long k, Short v) {
			Ent g = this;
	 ...
	 * @param loadFactor
	 */
	public LongShortHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	 ...
	 * @param loadFactor
	 */
	public LongShortHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	 ...
	 */
	public LongShortHashMap(LongShortMap mp) {
		this(16, 0.75f);
		putAllElement(mp);
	}
	 ...
	 */
	public Set<LongShortMap.PrimitiveEntry> primitiveEntrySet() {
		if(entrySet == null) {
	...
	//
	private boolean _putEntry(long k, Short v) {
		int h = getHash(k) % ent.length;
