...
implements java.io.Serializable {
	...
	private static class Ent<K> implements PrimitiveEntryV<K> {
		...
		//
		private Long putEntry(K k, long v) {
			Ent<?> g = this;
			
			for(Ent<?> f = this.next; f != null; g = f, f = f.next) {
				if((f.key == null && k == null) ||
						(k != null && k.equals(f.key))) {
					Long r = Long.valueOf(f.value);
	...
	public LongValueHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	...
	private class ESet extends AbstractSet<PrimitiveEntryV<K>> {
		...
		public Iterator<PrimitiveEntryV<K>> iterator() {
			return new Iterator<PrimitiveEntryV<K>>() {
				...
				public LongValueMap.PrimitiveEntryV<K> next() {
					remove = e;
					if(e == null) {
				...
				public void remove() {
					...
					LongValueHashMap.this.remove(e.key);
					exModCount = modCount;
					remove = null;
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.op.ToLongMap#entrySet()
	 */
	public Set<LongValueMap.PrimitiveEntryV<K>> longValueEntrySet() {
	...
	private class KSet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return LongValueHashMap.this.remove(o) != null;
		}
	...
	//
	@SuppressWarnings("unchecked")
	private void _putEntry(K k, long v) {
		int h = getHash(k) % ent.length;
	...
	private void readObject(
		...
		for(int i = 0; i < size; i++) {
			_putEntry((K)s.readObject(), s.readLong());
		}
	}
