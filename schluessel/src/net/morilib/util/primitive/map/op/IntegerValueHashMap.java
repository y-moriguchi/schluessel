 ...
 */
public class IntegerValueHashMap<K> extends AbstractIntegerValueMap<K>
implements java.io.Serializable {
	...
	private static class Ent<K> implements PrimitiveEntryV<K> {
		...
		private Integer putEntry(K k, int v) {
			...
			for(Ent<?> f = this.next; f != null; g = f, f = f.next) {
				if((f.key == null && k == null) ||
						(k != null && k.equals(f.key))) {
					Integer r = Integer.valueOf(f.value);
	...
	public IntegerValueHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	...
	public IntegerValueHashMap(int initialCapacity) {
		this(initialCapacity, 0.75f);
	}

	public IntegerValueHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	...
	private class ESet extends AbstractSet<PrimitiveEntryV<K>> {
		...
		public Iterator<PrimitiveEntryV<K>> iterator() {
			return new Iterator<PrimitiveEntryV<K>>() {
				...
				public void remove() {
					...
					IntegerValueHashMap.this.remove(e.key);
					exModCount = modCount;
					remove = null;
				}
		...
		public void clear() {
			IntegerValueHashMap.this.clear();
		}

	}

	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.op.ToIntegerMap#entrySet()
	 */
	...
	private class KSet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return IntegerValueHashMap.this.remove(o) != null;
		}
	...
	private void readObject(
		...
		for(int i = 0; i < size; i++) {
			_putEntry((K)s.readObject(), s.readInt());
		}
	}
