 ...
 */
public class ShortValueHashMap<K> extends AbstractShortValueMap<K>
implements java.io.Serializable {
	...
	private static class Ent<K> implements PrimitiveEntryV<K> {
		...
		//
		private Short putEntry(K k, short v) {
			Ent<?> g = this;
			
			for(Ent<?> f = this.next; f != null; g = f, f = f.next) {
				if((f.key == null && k == null) ||
						(k != null && k.equals(f.key))) {
					Short r = Short.valueOf(f.value);
	...
	public ShortValueHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	...
	public ShortValueHashMap(int initialCapacity) {
		this(initialCapacity, 0.75f);
	}
	
	
	public ShortValueHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	
	
	public ShortValueHashMap() {
		this(16, 0.75f);
	}
	...
	private class ESet extends AbstractSet<PrimitiveEntryV<K>> {
		...
		public Iterator<PrimitiveEntryV<K>> iterator() {
			return new Iterator<PrimitiveEntryV<K>>() {
				...
				public ShortValueMap.PrimitiveEntryV<K> next() {
					remove = e;
					if(e == null) {
				...
				public void remove() {
					...
					ShortValueHashMap.this.remove(e.key);
					exModCount = modCount;
					remove = null;
				}
		...
		public void clear() {
			ShortValueHashMap.this.clear();
		}
		
	}
	
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.op.ToShortMap#entrySet()
	 */
	public Set<ShortValueMap.PrimitiveEntryV<K>> shortValueEntrySet() {
	...
	private class KSet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return ShortValueHashMap.this.remove(o) != null;
		}
	...
	//
	@SuppressWarnings("unchecked")
	private void _putEntry(K k, short v) {
		int h = getHash(k) % ent.length;
	...
	private void readObject(
		...
		for(int i = 0; i < size; i++) {
			_putEntry((K)s.readObject(), s.readShort());
		}
	}
