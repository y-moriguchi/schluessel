 ...
 */
public class CharacterValueHashMap<K> extends AbstractCharacterValueMap<K>
implements java.io.Serializable {
	...
	private static class Ent<K> implements PrimitiveEntryV<K> {
		...
		//
		private Character putEntry(K k, char v) {
			Ent<?> g = this;
	...
	public CharacterValueHashMap(int initialCapacity, float loadFactor) {
		int cp = 1;
	...
	public CharacterValueHashMap(int initialCapacity) {
		this(initialCapacity, 0.75f);
	}
	
	public CharacterValueHashMap(float loadFactor) {
		this(16, loadFactor);
	}
	
	public CharacterValueHashMap() {
		this(16, 0.75f);
	}
	...
	private class ESet extends AbstractSet<PrimitiveEntryV<K>> {
		...
		public Iterator<PrimitiveEntryV<K>> iterator() {
			return new Iterator<PrimitiveEntryV<K>>() {
				...
				public CharacterValueMap.PrimitiveEntryV<K> next() {
					remove = e;
					if(e == null) {
				...
				public void remove() {
					...
					CharacterValueHashMap.this.remove(e.key);
					exModCount = modCount;
					remove = null;
				}
		...
		public void clear() {
			CharacterValueHashMap.this.clear();
		}
		
	}
	
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.op.ToCharacterMap#entrySet()
	 */
	public Set<CharacterValueMap.PrimitiveEntryV<K>> charValueEntrySet() {
	...
	private class KSet extends AbstractSet<K> {
		...
		public boolean remove(Object o) {
			return CharacterValueHashMap.this.remove(o) != null;
		}
	...
	//
	@SuppressWarnings("unchecked")
	private void _putEntry(K k, char v) {
		int h = getHash(k) % ent.length;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public char putElement(K k, char v) {
		int h = getHash(k) % ent.length;
		...
		if(ent[h] == null) {
			...
			Character r = ent[h].putEntry(k, v);
	...
	private void writeObject(
		...
		i = charValueEntrySet().iterator();
		while(i.hasNext()) {
			PrimitiveEntryV<K> e = i.next();
			
			s.writeObject(e.getKey());
			s.writeChar(e.getValue());
		}
	}
	...
	private void readObject(
		...
		for(int i = 0; i < size; i++) {
			_putEntry((K)s.readObject(), s.readChar());
		}
	}
