 ...
 */
public class ByteValueHashMap<K> extends AbstractByteValueMap<K>
implements java.io.Serializable {
		...
		public Iterator<PrimitiveEntryV<K>> iterator() {
			return new Iterator<PrimitiveEntryV<K>>() {
				...
				public void remove() {
					...
					ByteValueHashMap.this.remove(e.key);
					exModCount = modCount;
					remove = null;
				}
	...
	/* (non-Javadoc)
	 * @see net.morilib.util.primitive.map.op.ToByteMap#entrySet()
	 */
	...
	private void readObject(
		...
		for(int i = 0; i < size; i++) {
			_putEntry((K)s.readObject(), s.readByte());
		}
	}
