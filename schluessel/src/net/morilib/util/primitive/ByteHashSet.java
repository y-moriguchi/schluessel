 ...
 */
public class ByteHashSet extends AbstractByteSet
implements java.io.Serializable {
	...
	public ByteIterator byteIterator() {
		return new ByteIterator() {
			...
			public void remove() {
				...
				ByteHashSet.this.removeByte(remove.value);
				exModCount = modCount;
				remove = null;
			}
	...
	private void resize() {
		if(size > threshold && ent.length < MAXIMUM_CAPACITY) {
			...
			for(ByteIterator bi = byteIterator(); bi.hasNext();) {
				byte b = bi.next();
				int  h = getHash(b) % ns;
	...
	public boolean addAllByte(ByteCollection c) {
		...
		for(ByteIterator i = c.byteIterator(); i.hasNext();) {
