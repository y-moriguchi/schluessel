 ...
 */
public class LongitudeTest extends TC {

	Longitude deg(String s) {
		return Longitude.parse(s);
	}

	Longitude deg(int d, int m, int s, int us) {
		return new Longitude(d, m, s, us);
	}

	Longitude deg(int x) {
		return new Longitude(x);
	}

	public void testParse() {
		eq(deg("1°E"), deg(  1, 0, 0, 0));
		eq(deg("1°W"), deg( -1, 0, 0, 0));
		try {
			deg("1°");  fail();
		} catch(NumberFormatException e) {}
	}

	void eqadd(Longitude x, Longitude y, Longitude z) {
		eq(x.add(y), z);
	}
	...
	public void testAdd() {
		...
		eqadd(deg(-180,  0,  0,  0), deg(180,  0,  0,  0), deg(    0,  0,  0,  0));
		eqadd(deg( 180,  0,  0,  0), deg(180,  0,  0,  0), deg(    0,  0,  0,  0));
	}

	void eqsub(Longitude x, Longitude y, Longitude z) {
		eq(x.subtract(y), z);
	}
	...
	public void testSubtract() {
		...
		eqsub(deg(-180,  0,  0,  0), deg(180,  0,  0,  0), deg(    0,  0,  0,  0));
		eqsub(deg( 180,  0,  0,  0), deg(180,  0,  0,  0), deg(    0,  0,  0,  0));
	}
